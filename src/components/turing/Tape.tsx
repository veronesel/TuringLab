...
const getTapeSkinStyles = (
  ...
  if (tapeSkin === "default") {
    ...
    const typewriterBase =
      "font-serif text-3xl font-black bg-[#fdf6e3] text-[#2c1d11] border-2 border-[#d4c5b0] dark:bg-[#1a140b] dark:text-[#fbbf24] dark:border-[#4d3a1f] shadow-inner rounded-md";
    if (isHead) {
      skinStyles = `${typewriterBase} ring-2 ring-[#2c1d11] dark:ring-[#fbbf24] relative z-20`;
    } else {
      skinStyles = `${typewriterBase}`;
    }
    ...
    const dotStyle = twMerge(
      ...
      isBlnk
        ? "w-2.5 h-2.5 border-2 border-[#94a3b8] dark:border-[#e2e8f0]"
        : cellValue === "0"
          ? "w-3.5 h-3.5 bg-primary-base shadow-sm"
          : cellValue === "1"
            ? "w-5.5 h-5.5 bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]"
    ...
    skinStyles = twMerge(
      ...
      isHead
        ? "border-primary-base border-2 relative z-20 shadow-[0_0_10px_rgba(255,255,255,0.15)] bg-bg-panel"
        : "",
    );
    ...
    const binStr =
      cellValue === "_"
        ? "00000000"
        : cellValue.charCodeAt(0).toString(2).padStart(8, "0");
    const baseColors =
      "font-mono text-[9px] bg-[#070b12] border-2 border-[#14233c] text-emerald-400 font-bold uppercase tracking-wider rounded-md";
    if (isHead) {
      skinStyles = `${baseColors} border-emerald-500 shadow-[0_0_12px_rgba(16,185,129,0.5)] relative z-20 text-emerald-300`;
    } else {
      skinStyles = `${baseColors}`;
    }
    content = (
      <div className="flex flex-col items-center justify-center gap-0.5 leading-none pointer-events-none">
        <span className="text-[6.5px] text-emerald-300 font-sans tracking-wide font-black">
          BYTE
        </span>
        <span className="font-mono text-[8.5px] tracking-tight text-emerald-400 font-black">
          {binStr}
        </span>
      </div>
    );
...
export const Tape: React.FC = () => {
  ...
  const clearTapeAndResetHead = useTMStore(
    (state) => state.clearTapeAndResetHead,
  );
  const activeScenario = useTMStore((state) => state.activeScenario);
  const executionSpeed = useTMStore((state) => state.executionSpeed);
  const showExpectedOutcome = useThemeStore(
    (state) => state.showExpectedOutcome,
  );
  const showExecutionTimeline = useThemeStore(
    (state) => state.showExecutionTimeline,
  );

  // We load or save historical max steps to localStorage to provide empirical data
  const [historicalSteps, setHistoricalSteps] = useState<
  ...
  useEffect(() => {
    if (headPosition !== undefined && tapeTrackRef.current) {
      tapeTrackRef.current.scrollIntoView({
        behavior: "smooth",
        block: "nearest",
      });
    }
  ...
  const activeBorderPreset =
    BorderPresets.find((p) => p.value === customBorderColor) ||
    BorderPresets[0];
  const activeTextColorPreset =
    TextPresets.find((p) => p.value === customTextColor) || TextPresets[0];
  ...
  // Dynamic transition config for tape slide
  const slideTransition = useMemo<any>(() => {
    if (!isRunning) {
      // Gentle, luxurious spring when manually scrolling or editing
      return { type: "spring" as const, stiffness: 300, damping: 30 };
    }

    // During active simulation, scale stiffness so it stays synchronized with pace of steps
    // executionSpeed is ms per step. E.g. at 500ms, a snappy spring. At 50ms, extremely fast.
    if (executionSpeed < 80) {
      return {
        type: "tween" as const,
        duration: executionSpeed / 1000,
        ease: "easeInOut" as const,
      };
    } else if (executionSpeed < 180) {
      return { type: "spring" as const, stiffness: 800, damping: 35 };
    } else if (executionSpeed < 450) {
      return { type: "spring" as const, stiffness: 500, damping: 28 };
    } else {
      return { type: "spring" as const, stiffness: 300, damping: 24 };
    }
  }, [isRunning, executionSpeed]);

  const apertureTransition = useMemo<any>(() => {
    if (!isRunning) {
      return { type: "spring" as const, stiffness: 450, damping: 20 };
    }
    if (executionSpeed < 100) {
      return {
        type: "tween" as const,
        duration: executionSpeed / 1000,
        ease: "linear" as const,
      };
    } else if (executionSpeed < 300) {
      return { type: "spring" as const, stiffness: 650, damping: 22 };
    } else {
      return { type: "spring" as const, stiffness: 450, damping: 20 };
    }
  }, [isRunning, executionSpeed]);
  ...
  return (
    <div className="flex-1 flex flex-col relative select-none overflow-y-auto overflow-x-hidden border-2 border-primary-base/20 rounded-xl bg-bg-panel shadow-[0_4px_30px_rgba(0,0,0,0.3)] hover:border-primary-base/40 transition-all duration-500">
      ...
      <div
        ...
        style={{
          backgroundImage:
            "radial-gradient(color-mix(in srgb, var(--color-primary-base) 25%, transparent) 1px, transparent 1px)",
      ...
      <div className="w-full flex shrink-0 justify-between items-center px-4 py-3 relative z-30 flex-nowrap gap-x-4">
        <div className="flex items-center gap-2 flex-nowrap overflow-hidden shrink-0">
          <span className="text-[9px] text-text-muted font-bold uppercase tracking-widest hidden">
            History Scrubber
          </span>
          <button
            onClick={undo}
            disabled={historyIndex <= 0 || isRunning}
            className="p-1 rounded bg-bg-element hover:bg-border-active disabled:opacity-50 transition-colors"
            title="Undo Step"
          >
          ...
          <button
            ...
            className={clsx(
              ...
              infiniteMode
                ...
                : "bg-bg-element text-text-muted border-border-main",
            )}
          >
        ...
        <div className="flex items-center gap-3 ml-auto shrink-0 flex-nowrap">
          <div className="flex items-center gap-1 bg-bg-element rounded px-2 py-1 border border-border-main">
            ...
            >
              <option value="default" className="bg-bg-panel">
                Standard
              </option>
              <option value="typewriter" className="bg-bg-panel">
                Typewriter
              </option>
              <option value="dots" className="bg-bg-panel">
                Dots
              </option>
              <option value="binary" className="bg-bg-panel">
                Binary
              </option>
            </select>
          </div>
              ...
              >
                <Palette
                  ...
                  className={clsx(
                    ...
                    (customBorderColor !== "default" ||
                      customTextColor !== "default") &&
                      "animate-pulse text-primary-base",
                  )}
                />
                <span>Colors</span>
              </button>
                  ...
                  <div className="absolute right-0 mt-1.5 w-64 bg-bg-panel border border-border-main/80 rounded-lg shadow-2xl p-3.5 flex flex-col gap-4 z-50 font-sans">
                    ...
                    <div className="flex flex-col gap-2">
                      <span className="text-[10px] font-extrabold uppercase tracking-wide text-text-muted flex justify-between">
                        ...
                        <span className="text-[8px] text-text-faint normal-case">
                          Predefined sets
                        </span>
                      </span>
                        ...
                        {BorderPresets.map((preset) => (
                          <button
                            ...
                            className={clsx(
                              ...
                              customBorderColor === preset.value
                                ? "border-white ring-2 ring-primary-base"
          ...
          <span className="text-[9px] text-primary-base font-mono bg-bg-element px-2 py-1 rounded border border-border-main">
            STEP {historyIndex.toString().padStart(4, "0")}
          </span>
        </div>
      </div>
            ...
            {activeScenario && (
              <div className="flex items-center justify-between text-[9px] font-sans text-text-muted gap-2">
                <div className="truncate max-w-[85%]">
                  <span className="font-bold text-text-primary uppercase mr-1">
                    Outcome Target:
                  </span>
                  <span className="italic">
                    {expectedOutcomeText || activeScenario.description}
                  </span>
                </div>
      ...
      >
            ...
            <button
              ...
              className={twMerge(
                clsx(
                  ...
                  scrollOffset !== 0
                    ? "hover:scale-105 cursor-pointer active:scale-95 shadow-md opacity-100 animate-pulse"
              ...
              style={{
                ...
                boxShadow:
                  customBorderColor !== "default"
                    ? `0 0 10px ${activeBorderPreset.colorCode}`
                    : "0 0 10px var(--color-primary-base)",
              }}
              title={
                scrollOffset !== 0
                  ? "Click to recenter tape on the head pointer"
                  : undefined
              }
            >
              HEAD: {headPosition}
        ...
        <div className="absolute left-1/2 top-1/2 -translate-y-1/2 w-0 h-16 pointer-events-none">
          ...
          >
            ...
            {hoveredCell !== null &&
              hoveredCell !== headPosition &&
              !isRunning && (
                <div
                  className="absolute top-[-30px] z-20 flex flex-col items-center pointer-events-none transition-all duration-75 -translate-x-1/2"
                  style={{ left: hoveredCell * 56 }}
                >
                  <div className="text-[10px] font-mono font-black text-text-primary dark:text-white mb-1 tracking-widest bg-bg-panel border border-border-main rounded px-1.5 py-0.5 shadow-md">
                    EDIT
                  </div>
                  <div className="w-0 h-0 border-l-[6px] border-l-transparent border-r-[6px] border-r-transparent border-t-[8px] border-t-text-primary"></div>
                </div>
              )}
            ...
            {cells.map((cell) => {
              ...
              const colorConfig =
                MarkerColors.find((c) => c.value === bookmarkColor) ||
                MarkerColors[0];

              const isHovered =
                hoveredCell === cell.index && !isHead && !isRunning;
              const isSelected = selectedIndices.includes(cell.index);

              const aliasText =
                symbolAliases[cell.value === "_" ? "_" : cell.value];
              const { skinStyles, content } = getTapeSkinStyles(
                tapeSkin,
                cell.value,
                isHead,
                headPosition,
              ...
              if (tapeSkin === "default") {
                ...
                const dist = Math.abs(cell.index - headPosition);
                if (dist === 0) {
                  targetScale = 1.18;
                  targetY = -5;
                } else if (dist === 1) {
              ...
              return (
                ...
                >
                    ...
                    {isBookmarked && (
                      ...
                      >
                        <div
                          ...
                          title={
                            bookmarkNote
                              ? `${bookmarkIcon} ${bookmarkNote} (Click to jump, Drag to move)`
                              : `${bookmarkIcon} Bookmarked (Click to jump, Drag to move)`
                          }
                        >
                          <span className="shrink-0 text-[10px] leading-none mb-[1.5px]">
                            {bookmarkIcon}
                          </span>
                          {bookmarkNote && (
                  ...
                  {!isBookmarked && (
                    <div
                      className="absolute -bottom-4 w-2 h-2 rotate-45 bg-[#3b82f6] opacity-0 group-hover:opacity-100 transition-opacity"
          ...
          <div className="flex justify-between items-center mb-2.5">
            <div className="flex items-center gap-2">
              <Film
                ...
                className={clsx(
                  "text-primary-base",
                  isRunning && "animate-[spin_4s_linear_infinite]",
                )}
              />
              <span className="text-[10px] font-bold uppercase tracking-widest text-text-secondary">
                Execution Spectrum Timeline
              </span>
              <span className="text-[10px] text-text-secondary font-black">•</span>
              <div className="flex items-center gap-1.5 text-[10px] font-mono text-text-primary font-bold">
                <span>Step</span>
            ...
            <div className="flex items-center gap-2 text-[10px]">
              {history[historyIndex] && (
                <div className="flex items-center gap-1.5 bg-bg-element/40 border border-border-main/40 px-2 py-0.5 rounded-md">
                  <span className="text-text-muted text-[9px] uppercase tracking-wider font-extrabold mr-0.5">
                    Machine State:
                  </span>
                  <span
                    ...
                    style={{
                      backgroundColor:
                        activeScenario?.stateColors?.[
                          history[historyIndex].currentState
                        ] || "#3b82f6",
                    }}
                  />
                  <code className="font-mono font-bold text-primary-base">
                    {history[historyIndex].currentState}
                  </code>
                </div>
              )}
              ...
              {history[historyIndex]?.lastRuleId ? (
                <div className="hidden sm:flex items-center gap-1 bg-[#1c2128] border border-border-main px-2 py-0.5 rounded text-[8px] text-text-faint font-mono">
                  <span className="font-bold text-text-faint uppercase font-sans">
                    Last Rule ID:
                  </span>
                  <span className="text-primary-base font-bold">
                    {history[historyIndex].lastRuleId?.slice(0, 8)}...
                  </span>
                </div>
              ) : (
          ...
          <div className="relative w-full h-8 flex items-center group/timeline rounded-lg overflow-hidden bg-bg-element border border-border-main/60 shadow-inner">
            ...
            <div className="absolute inset-0 flex w-full h-full pointer-events-none opacity-45 group-hover/timeline:opacity-65 transition-opacity">
              {history.map((histEntry, idx) => {
                const stateColor =
                  activeScenario?.stateColors?.[histEntry.currentState] ||
                  "#3b82f6";
            ...
            <div className="absolute inset-x-0 bottom-1 flex w-full pointer-events-none px-1">
              {history.map((histEntry, idx) => {
                const hasBookmarkAtHead =
                  bookmarks[histEntry.headPosition] !== undefined;
                if (!hasBookmarkAtHead)
                  return <div key={idx} className="flex-1 h-1" />;

                return (
                  <div
                    key={idx}
                    className="flex-1 flex justify-center items-center h-1"
                  >
                    <div
                      className="w-1.5 h-1.5 rounded-full bg-amber-400 border border-black/40 shadow-sm animate-pulse"
                      title={`Marker at Step ${idx}`}
                    />
                  </div>
                );
              })}
          ...
          <div className="flex justify-between items-center mt-1.5 text-[8.5px] text-text-faint font-mono uppercase tracking-wide px-1">
            <div className="flex gap-4">
              ...
              <span className="font-bold">
                Speed: {Math.round(2000 / useTMStore.getState().executionSpeed)}
                x
              </span>
            </div>
            ...
            <MapPin
              ...
              className={clsx(
                ...
                Object.keys(bookmarks).length > 0
                  ? "text-primary-base animate-pulse"
                  : "text-text-muted",
              )}
            />
          ...
          >
            ...
            <button
              onClick={() => {
                setEditingBookmark({
                  index: headPosition,
                  note: parseBookmark(bookmarks[headPosition]).note,
                  icon: parseBookmark(bookmarks[headPosition]).icon || "📍",
                  color: parseBookmark(bookmarks[headPosition]).color || "blue",
            ...
            {Object.keys(bookmarks).length > 0 && (
              <button
                onClick={() => {
                  if (
                    window.confirm(
                      "Are you sure you want to clear all visual markers?",
                    )
                  ) {
                ...
                {Object.keys(bookmarks).length === 0 ? (
                  <div className="text-center py-4 text-xs text-text-faint italic flex flex-col items-center gap-1 font-sans">
                    ...
                    <span className="text-[10px] text-text-muted">
                      Click on any cell of the tape or use the "+ Marker at
                      Head" button to add landmarks.
                    </span>
                  </div>
                ) : (
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 font-sans">
                    {Object.keys(bookmarks)
                      ...
                      .map((idx) => {
                        ...
                        const colorConfig =
                          MarkerColors.find((c) => c.value === color) ||
                          MarkerColors[0];
                            ...
                            >
                              <div
                                className={clsx(
                                  ...
                                  isAtHead
                                    ? "bg-primary-base text-bg-base"
                                    : "bg-bg-element text-text-primary",
                                )}
                              >
                                {idx}
                              </div>
                              ...
                              <div className="flex flex-col min-w-0">
                                <div className="flex items-center gap-1.5">
                                  ...
                                  {isAtHead && (
                                    <span
                                      className="w-1.5 h-1.5 rounded-full bg-primary-base animate-pulse shrink-0"
                                      title="Active Head is here"
                                    />
                                  )}
                                </div>
                                <p
                                  className="text-[10px] text-text-primary truncate font-medium mt-0.5"
                                  title={note || `Cell #${idx}`}
                                >
                                  {note || (
                            ...
                            <div className="flex items-center gap-1 pl-1 shrink-0 font-sans">
                              <button
                                onClick={() => {
                                  setEditingBookmark({
                                    index: idx,
                                    note: parseBookmark(bookmarks[idx]).note,
                                    icon:
                                      parseBookmark(bookmarks[idx]).icon ||
                                      "📍",
                                    color:
                                      parseBookmark(bookmarks[idx]).color ||
                                      "blue",
                                    symbol: tape[idx] || "_",
                                    isMarker: true,
                                  });
              ...
              <div className="flex justify-between items-center border-b border-border-main pb-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold uppercase tracking-wider text-text-primary">
                    Configure Cell #{editingBookmark.index}
                  </span>
                </div>
                <button
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      maxLength={1}
                      value={
                        editingBookmark.symbol === "_"
                          ? ""
                          : editingBookmark.symbol
                      }
                      onChange={(e) => {
                ...
                <div className="flex flex-col gap-2 mt-1">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <label
                      htmlFor="is-marker-cb"
                      className="text-[10px] font-bold uppercase tracking-wider text-text-muted cursor-pointer select-none"
                    >
                  ...
                  {editingBookmark.isMarker && (
                    <div className="flex flex-col gap-1 pl-5">
                      <label className="text-[9px] font-semibold text-text-faint">
                        Marker Label/Note (Optional)
                      </label>
                      <input
                        type="text"
                          ...
                          {MarkerIcons.map((iconSymbol) => (
                            <button
                              ...
                              className={clsx(
                                ...
                                editingBookmark.icon === iconSymbol
                                  ? "bg-primary-base/20 border border-primary-base"
                                  : "bg-bg-element border border-border-main/40",
                              )}
                            >
                              {iconSymbol}
                            </button>
                          ...
                          {MarkerColors.map((col) => (
                            <button
                              ...
                              className={clsx(
                                ...
                                editingBookmark.color === col.value
                                  ? "ring-2 ring-primary-base border-white"
                                  : "border-transparent",
                              )}
                              style={{
                  ...
                  onClick={() => {
                    ...
                    if (editingBookmark.isMarker) {
                      addBookmark(
                        editingBookmark.index,
                        serializeBookmark(
                          editingBookmark.note,
                          editingBookmark.icon || "📍",
                          editingBookmark.color || "blue",
                        ),
                      );
                    } else {
